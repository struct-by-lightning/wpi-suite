...
public class UserDeserializer implements JsonDeserializer<User> {
	...
	public User deserialize(JsonElement userElement, Type userType,
		...
		if (deflated.has("password")
				&& !deflated.get("password").getAsString().equals("")) {
			password = deflated.get("password").getAsString();
		}

		if (deflated.has("name")
				&& !deflated.get("name").getAsString().equals("")) {
			name = deflated.get("name").getAsString();
		}

		if (deflated.has("email")
				&& !deflated.get("email").getAsString().equals("")) {
		...
		if (deflated.has("role")
				&& !deflated.get("role").getAsString().equals("")) {
